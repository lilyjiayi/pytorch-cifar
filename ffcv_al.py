...
from ffcv.transforms import ToTensor, ToDevice, ToTorchImage, RandomHorizontalFlip, NormalizeImage
...
def main():
    ...
    parser.add_argument('--ischedule', default="cosine", type=str, choices=['constant', 'cosine', 'find_lr']) 
    parser.add_argument('--schedule', default="cosine", type=str, choices=['constant', 'cosine']) 
    ...
    args.drop_last = True
    # whether to pin memory or not for dataloaders
   ...
   # create checkpoing save_name
    ...
    train_data = WILDSSubset(train_data, range(500000), transform=None)
    train_val_data = WILDSSubset(train_val_data, range(500000), transform=None)
    raw_val_data = WILDSSubset(raw_val_data, range(120000), transform=None)
    
    # print out info about train/val data
    ...
    if args.resume:
        ...
        for i in range(args.inum_epoch):
            ...
            if not(query_end):
                curr_test_acc, val_probs, val_losses = test(epoch, net, val_data, val_loader, grouper, full_grouper, label_grouper, 
                                                        args.log_op, criterion, device, False, curr_query)
            epoch += 1
...
def ffcv_train_loader(indices=None, num_workers=8, batch_size=128, drop_last=True, pin_memory=True):
    target_resolution = (224, 224)
...
def ffcv_val_loader(indices=None, num_workers=8, batch_size=128, pin_memory=True):
    target_resolution = (224, 224)
    eval_image_pipeline = [
    ...
    val_loader = Loader('/self/scr-sync/nlp/geo_yfcc_ffcv/one_label_val.beton',
            batch_size=batch_size,
            num_workers=num_workers,
