...
def main():
    ...
    # args.root_dir = '/self/scr0/jiayili'
    if args.dataset == 'celebA':
        dataset = get_dataset(dataset=args.dataset, download=True, root_dir = args.root_dir, target='Blond_Hair', group=['Male'])
    else:
...
def test(epoch, net, dataset, dataloader, grouper, criterion, device, best_acc, group_strategy,
    ...
    if group_strategy=='avg_c_val':
       worst_group = get_avgc_worst_group(confidences, grouper, dataset)
    ...
    #     print('Saving..')
    #     best_acc = acc
    # if save and query_end and curr_query % 10 == 0:
    #     print('Saving..')
    #     state = {
    #         'net': net.state_dict(),
    #         'acc': acc,
